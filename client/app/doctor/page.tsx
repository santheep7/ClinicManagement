...
export default function Home() {
  // Seed from localStorage cache so the branding panel renders correctly
  // immediately on client-side navigation (e.g. after logout), not just on hard refresh.
  const [activeClinics, setActiveClinics] = useState<Clinic[]>(() => {
    if (typeof window === "undefined") return [];
    try {
      const cached = localStorage.getItem("cachedClinics");
      return cached ? (JSON.parse(cached) as Clinic[]) : [];
    } catch { return []; }
  });
  const [clinicsLoading, setClinicsLoading] = useState(true);

  // Also restore the last-used clinicId from the user record so the panel
  // shows the right branding the moment the login page mounts after logout.
  const [suClinicIdInit] = useState<string>(() => {
    if (typeof window === "undefined") return "";
    try {
    ...
    async function fetchClinics() {
      try {
        ...
        if (res.ok && data.clinics) {
          ...
          // Keep the cache fresh for the next client-side navigation
          localStorage.setItem("cachedClinics", JSON.stringify(data.clinics));
        }
      } catch {
  ...
  const [suClinicId, setSuClinicId] = useState(suClinicIdInit);
